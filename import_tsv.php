...
$sql = "SELECT * FROM advertisers where active = 1 and googlesheetlocation != ''";
$result = $conn->query($sql);
if (!$result) {
...
#echo "imported from the following TSV files....<br>";
 ...
 while($row = $result->fetch_assoc()) {
	...
	// delete todays records from the DEDUPED (final) table...
		...
		$sql = "DELETE FROM  `gs_url_dd` WHERE DATE_FORMAT( recdate,  '%Y-%m-%d' ) = CURDATE( )";
		$del2_result = $conn->query($sql);
		
	// insert UNIQUE URLs to the DEDUPED table...
	
		$sql = "insert into gs_url_dd (aid,accountname,campaign, adgroup,adgroupid,url) select distinct aid,accountname,campaign, adgroup,adgroupid,url from gs_url";
